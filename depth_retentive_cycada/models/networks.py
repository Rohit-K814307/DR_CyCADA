...
def prediction_dr(out, target=None):
    if target is None:
        return out
    else:
        acc = (out[0] == target[0]).sum().item() + (out[1] == target[1]).sum().item() + (out[2] == target[2]).sum().item()
        acc = acc / ((target[0].size()[0] **2) * 3)
        return out, acc
...
def define_P(model_type="DPT_Large"):
    ...
    return torch.hub.load("intel-isl/MiDaS",model_type).train(), transform
