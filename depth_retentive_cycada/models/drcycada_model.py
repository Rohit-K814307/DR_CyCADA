...
class DRCyCADAModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_dpt_A', type=float, default=1.0, help='weight for semantic loss (A -> B -> A)')
            parser.add_argument('--lambda_dpt_B', type=float, default=1.0, help='weight for semantic loss (B -> A -> B)')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A', 'fake_B', 'rec_A', 'real_B', 'fake_A', 'rec_B', 
                             'depth_fake_A', 'depth_fake_B','depth_real_A', 'depth_real_B']
        if self.isTrain:
            # specify the training losses you want to print out. The program will call base_model.get_current_losses
            self.loss_names = ['D_A', 'G_A', 'cycle_A', 'D_B', 'G_B', 'cycle_B', 'dpt_B', 'dpt_A', 'P_B_task', 'P_B_adv', 'P_B', 'D_ft_adv']
        ...
        self.netG_B = networks.define_G(opt.output_nc, opt.input_nc, opt.ngf, opt.netG, opt.norm,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netP_A, self.transform = networks.define_P(model_type="MiDaS_small")
        self.netP_B, _ = networks.define_P(model_type="MiDaS_small")
        self.netP_A = self.netP_A.to(self.device).eval()
        self.netP_B = self.netP_B.to(self.device)
        self.netD_ft = networks.define_C(1, "d_dpt", opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.criterionDFT = torch.nn.CrossEntropyLoss()
            ...
            self.optimizer_P_B = torch.optim.Adam(self.netP_B.parameters(), lr=opt.c_lr/10, betas=(opt.beta1, 0.999))
    ...
    def P_viz(self, netP, img):
        with torch.no_grad():
            prediction = netP(img)
            prediction = self.P_norm(prediction)
        return prediction
    ...
    def backward_P_A(self):
        ...
        self.loss_P_A = self.criterionDPT(self.P_norm(self.netP_A(self.real_A)).unsqueeze(0), self.target_A) # P_A(A)
        self.loss_P_A.backward()

    def pretrain(self):
        # forward
        self.pred_real_A = self.P_norm(self.netP_A(self.real_A)).unsqueeze(0)
        label_real_A, score_acc_real_A = (self.pred_real_A == self.target_A).float().sum()
        # P_A
        self.set_requires_grad([self.netP_A], True)
        self.optimizer_P_A.zero_grad()
        self.backward_P_A()
        self.optimizer_P_A.step()
        return score_acc_real_A, self.loss_P_A
    ...
    def forward(self):
        self.fake_A = self.P_norm(self.netG_B(self.real_B)) # G_B(B)
        self.fake_B = self.P_norm(self.netG_A(self.real_A)) # G_A(A)
        self.rec_A = self.P_norm(self.netG_B(self.fake_B))# G_B(G_A(A))
        self.rec_B = self.P_norm(self.netG_A(self.fake_A))  # G_A(G_B(B))

        self.pred_real_A = self.P_norm(self.netP_A(self.real_A)).unsqueeze(0)
        self.pred_real_B = self.P_norm(self.netP_B(self.real_B)).unsqueeze(0)
        self.pred_fake_A = self.P_norm(self.netP_A(self.fake_A)).unsqueeze(0)
        self.pred_fake_B = self.P_norm(self.netP_B(self.fake_B)).unsqueeze(0)
        self.pred_rec_A = self.P_norm(self.netP_A(self.rec_A)).unsqueeze(0)
        self.pred_rec_B = self.P_norm(self.netP_B(self.rec_B)).unsqueeze(0)

        self.depth_real_A = self.P_viz(self.netP_A,self.real_A.detach()).unsqueeze(0).cpu()
        self.depth_real_B = self.P_viz(self.netP_B,self.real_B.detach()).unsqueeze(0).cpu()
        self.depth_fake_A = self.P_viz(self.netP_A,self.fake_A.detach()).unsqueeze(0).cpu()
        self.depth_fake_B = self.P_viz(self.netP_B,self.fake_B.detach()).unsqueeze(0).cpu()

        self.score_acc_real_A = 1.0
        self.score_acc_real_B = 1.0
        self.score_acc_fake_A = (self.pred_fake_A == self.target_A).float().sum().item() / self.target_A.size()[0]
        self.score_acc_fake_B = (self.pred_fake_B == self.target_B).float().sum().item() / self.target_B.size()[0]
        self.score_acc_rec_A = (self.pred_rec_A == self.target_A).float().sum().item() / self.target_A.size()[0]
        self.score_acc_rec_B = (self.pred_rec_B == self.target_B).float().sum().item() / self.target_B.size()[0]

        self.score_acc_depth_real_A = self.score_acc_real_A
        self.score_acc_depth_real_B = self.score_acc_real_B
        self.score_acc_depth_fake_A = self.score_acc_fake_A
        self.score_acc_depth_fake_B = self.score_acc_fake_B
        self.score_acc_depth_rec_A = self.score_acc_rec_A
        self.score_acc_depth_rec_B = self.score_acc_rec_B
        
        pred_source = self.netD_ft(self.pred_fake_B)
        pred_target = self.netD_ft(self.pred_real_B)
    ...
    def backward_G(self):
        ...
        # semantic loss A
        self.loss_dpt_A = self.criterionDPT(self.P_norm(self.netP_A(self.fake_A)).unsqueeze(0), self.P_norm(self.netP_A(self.real_B)).unsqueeze(0))
        # semantic loss B
        self.loss_dpt_B = self.criterionDPT(self.P_norm(self.netP_A(self.fake_B)).unsqueeze(0), self.P_norm(self.netP_A(self.real_A)).unsqueeze(0))
        self.loss_G += self.loss_dpt_A + self.loss_dpt_B
        self.loss_G.backward()

    def backward_D_ft(self):
        # Source
        pred_source = self.netD_ft(self.P_norm(self.netP_A(self.fake_B.detach())).unsqueeze(0))
        loss_D_ft_s = self.criterionDFT(pred_source, self.source_label)
        # Target
        pred_target = self.netD_ft(self.P_norm(self.netP_B(self.real_B)).unsqueeze(0))
        loss_D_ft_t = self.criterionDFT(pred_target, self.target_label)
        # Combined loss
        self.loss_D_ft_adv = (loss_D_ft_s + loss_D_ft_t) * 0.5
        self.loss_D_ft_adv.backward()

    def backward_P_B(self):
        pred_target = self.P_norm(self.netP_B(self.fake_B.detach())).unsqueeze(0) # C_B(G_A(A))
        self.loss_P_B_task = self.criterionDPT(pred_target, self.target_A)
        # only update net if discriminator is strong
        if self.score_acc_D_ft > 0.6:
            pred_target = self.netD_ft(self.P_norm(self.netP_B(self.real_B)).unsqueeze(0)) # D_ft(C_B(B))
            target_label = torch.ones(self.real_B.size()[0]).long().to(self.device)
            self.loss_P_B_adv = self.criterionDFT(pred_target, target_label)
        else:
            self.loss_P_B_adv = 0
        self.loss_P_B = self.loss_P_B_task + self.loss_P_B_adv
