...
class ClassUnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.dir_depth = os.path.join(opt.dataroot, opt.phase + 'depth_A')
        ...
        self.A_depth_paths = [(self.dir_depth + "/" + i.split(os.path.join(opt.dataroot, opt.phase + 'A'))[1][1:].split(".")[0] + ".txt").replace(opt.phase+"depth", opt.phase+"_depth") for i in self.A_paths]
        self.B_depth_paths = [(self.dir_depth + "/" + i.split(os.path.join(opt.dataroot, opt.phase + 'B'))[1][1:].split(".")[0] + ".txt").replace(opt.phase+"depth", opt.phase+"_depth") for i in self.B_paths]
    ...
    def __getitem__(self, index):
        ...
        A_depth = np.genfromtxt(self.A_depth_paths[index % self.A_size], delimiter=' ')
        B_depth = np.genfromtxt(self.B_depth_paths[index_B], delimiter=' ')
        # apply image transformation
        A = self.transform_A(A_img)
        B = self.transform_B(B_img)
        # TODO: in some cases, might need to implemenet target transfrom

        return {'A': A, 'B': B, 'A_paths': A_path, 'B_paths': B_path, 'A_target': torch.from_numpy(A_depth).unsqueeze(0), 'B_target': torch.from_numpy(B_depth).unsqueeze(0)}
